...
if (contactForm) {
  contactForm.addEventListener("submit", function (e) {
    ...
    emailjs
      .send("service_n8uzhqi", "template_jyarjnm", templateParams)
      .then(
        function (response) {
        ...
        function (error) {
          ...
          formStatus.innerHTML =
            "❌ <strong>Oops! Something went wrong.</strong><br>Please try again or contact us directly.";
...
async function loadServices() {
  try {
    ...
    console.log("✅ Services loaded successfully:", Object.keys(servicesMap));
  } catch (err) {
  ...
  if (service.why_us) {
    whyUsEl.innerHTML = `
      ...
      <div class="why-us-list">
        ${service.why_us
          .map((reason) => `<div class="why-us-item">${reason}</div>`)
          .join("")}
      </div>`;
  }
...
document.addEventListener("DOMContentLoaded", async () => {
  await loadServices();
  // Handle service card clicks (entire card)
  document.querySelectorAll(".service-card").forEach((card) => {
    card.addEventListener("click", (e) => {
      // Don't trigger if clicking on the See More button
      if (e.target.classList.contains("btn-see-more")) {
        return;
      }
      const key = card.getAttribute("data-service");
      console.log("🔍 Service card clicked, key:", key);
      console.log("🔍 Available services:", Object.keys(servicesMap));
  ...
  document.querySelectorAll(".btn-see-more").forEach((button) => {
    button.addEventListener("click", (e) => {
      ...
      const key = card.getAttribute("data-service");
      console.log("🔍 See More button clicked, key:", key);
      const service = servicesMap[key];
      console.log("🔍 Service found:", service);
      if (service) openServiceModal(service);
    });
